...
describe('Crypto Functions', () => {
  ...
  describe('validateFingerprintComponents', () => {
    ...
    it('should reject missing user agent', () => {
      ...
      expect(result.errors).toContain('Missing required component: userAgent');
    });

    it('should reject bot-like user agents', () => {
      const components = {
        userAgent: 'bot/1.0 crawler spider',
      };
      
      const result = validateFingerprintComponents(components);
      
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('User agent indicates automated client');
    });
    ...
    it('should reject invalid timezone offset', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Invalid timezone offset');
    });
  });
  ...
  describe('validateTimestamp', () => {
    ...
    it('should reject timestamp outside tolerance', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.error).toContain('Timestamp outside acceptable range');
    });
  });
});
